...
替换 `apps/mobile/views/profile.vue` 占位页，实现与原型图一致的**「我的」页**：浅蓝头部 + 蓝色渐变用户卡 + 9 项功能菜单（彩色图标 + 右箭头）+ 末尾 2 个设置项（适老模式 / 户外强光皮肤）。菜单点击走轻提示占位；户外强光开关实际切换根节点 `data-skin="outdoor"`（脚手架期 H5 降级实现）。

## 2. 范围与决策（已与用户确认）

- 入口：替换现有 `/profile`（与底栏「我的」标签对应），保持现有 3 入口底栏（首页 / 消息 / 我的）。
- 头部：**偏离** `docs/UI规范-移动端.md` 模板的「白底顶栏」，按原型图采用 `--primary-mobile-soft` 浅蓝背景（仅本页独立类，不污染 home/messages）。
- 用户卡：渐变蓝底白字（原型图效果），需新增 token。
...
| 文件                                | 动作 | 职责                                                                                            |
| ----------------------------------- | ---- | ----------------------------------------------------------------------------------------------- |
| `src/styles/tokens.css`             | 改   | 移动端块新增 `--mb-usercard-bg`（渐变）、`--mb-menu-contact-soft` 等 4 色浅底（复用语义色派生） |
| `apps/mobile/views/profile.vue`     | 重写 | 浅蓝头部 + 用户卡 + 9 菜单 + 2 设置 + 底栏；内联菜单/设置数据数组                               |
...
- 头部：高度 48px，浅蓝 `--primary-mobile-soft` 背景（独立类 `.profile-header`，不覆写 `.mb-brand-header`）；左中标题「我的」主色字，右「导航」图标按钮（跳 `/tasks`，同 home/messages）。
...
- **户外强光皮肤** `[data-skin='outdoor']`：用户卡渐变 → 白底黑字（`.mb-user-card` 覆盖 `background: var(--card-mobile); color:#000`）；浅蓝头部 → 白底黑字（`[data-skin='outdoor'] .profile-header` 覆盖）；遵守规范「全面停用渐变」。
