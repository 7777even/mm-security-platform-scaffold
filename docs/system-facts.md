...
- 2026-09-16（AI 过程治理补齐）：① 本库 `AGENTS.md` 新增 **§0「跨库入口」**（复述仓库根伞文件，因根 `mm-security-platform/` 不是 git 仓库、伞文件不入库，clone 单库曾读不到跨库约束）；② `.cursor/rules/` 新增 `openapi-contract-writing.mdc`（契约四条铁律与四同步顺序纪律由 `templates/` 迁一份为 harness 自动加载项）；③ 新增 `engineering/reports/`（干系人阶段简报层，与 `qa/`+`retro/` 分工）+ `templates/_stage_report_template.md`；④ 后端 `docs/architecture/alarm-three-surfaces.md` 固化报警三端口径，**与本文 §6.1 互为两端口**，任一侧改动须同步另一侧；⑤ 后端 `docs/AI编程规范与过程资产对标分析.md` 的 P1-1（Testcontainers / DB-IT）已由 ✅ 改判 🟡——本项目 DB-IT 层仍为缺口。
