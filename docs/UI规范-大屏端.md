...
> 端间边界红线（大屏态势禁止后台白底卡片 / 浅底标签 / 管理 `admin-*` 容器）见 `docs/UI规范-门户与后台边界.md`。
...
- **二级子应用全屏页**：路由同时满足 `meta.subapp === true && meta.hidden === true`（如 `/fire/rescue`）时，判定为全屏子应用，`AppLayout` 的 `isFullscreenSubapp` 生效 —— **收起主壳顶栏与底部消息栏**，仅保留子应用自身页头，地图铺满视口；**两个条件必须同时成立**（仅 `hidden` 的二级页如系统管理子页仍需保留主壳框架）。回归主模块页（如 `/fire`）时顶栏 / 模块导航 / 底部消息栏全部恢复。该行为不可下放到子应用内部实现，否则主壳与子应用双头叠加。
...
## 8. 端间互斥边界（引用 AGENTS §6.3 红线为权威源）

- **MUST NOT** 大屏态势端出现后台白卡容器：禁止套用 `admin-*` 风格白底卡片、浅灰页面底作为主视觉。
- 大屏是深色态势端，玻璃质感 / 渐变 / 发光是其专属语言；后台的浅色办公语言（白卡、浅灰底、实色平涂按钮）**禁止**出现在大屏（见 §1 第 8 条、§7 自检）。
- 端间互斥规则以 `AGENTS.md` §6.3 红线为权威源，本小节仅列大屏端具体禁项。
