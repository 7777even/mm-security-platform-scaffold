...
6. **MUST** 同屏字号 **≥ 7 档**（大屏是唯一允许多档位的端，见 §3.1），数值突出、标题加粗、说明缩小。
...
- **底部栏** `--layout-bottom-h: 56px`：按需承载时间轴 / 快捷工具。
- **四层信息架构**：全局控制层（顶部）→ 地图主视觉层（中央）→ 专题信息层（两侧卡片 / 列表 / 图表）→ 详情操作层（弹窗 / 抽屉 / 浮层，单点详情与处置操作）。
- **地图工具栏**：宽 `320–420px` 常驻或半常驻，提供图层、区域、搜索、测距测面、三维视角等工具。
- **页面边距**：`--layout-page-pad: var(--space-md)`（16px）。

### 3.1 字号阶梯（≥ 7 档，大屏专属多档位）
...
- [ ] 同屏字号 ≥ 7 档且来自 §3.1 表；z-index 只用五层 token。
