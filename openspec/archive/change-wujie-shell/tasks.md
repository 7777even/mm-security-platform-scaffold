...
- [x] 渐进迁移其余业务模块为子应用（复用同 WujieHost 机制）。已完成 3 个一级模块：fire-alarm / industrial-video / security-anti-terror 均登记为 wujie 子应用，每个含 `subapps/<module>/index.html` + `main.ts` 入口，并在 `vite.config.ts` 的 `rollupOptions.input` 登记同源多入口、在 `menu.ts` 的 `MenuRouteSpec` 标记 `subapp:true` + `subappUrl`。子应用内路由跳转经新增共享 `createSubappRouter()`（位于 `src/shell/subappRouter.ts`）委托主壳：其 `router.push` 改经 wujie 总线 emit 新增事件 `route-navigate`，主壳 `src/main.ts` 中 `WujieVue.bus.$on('route-navigate')` 监听后执行 `router.push`，承载二级页（如 `/fire-alarm/records`）。plans 为 dashboard 二级页（已在 dashboard 沙箱域内），system-users 为 system 分组占位页（主壳 `SECONDARY_ROUTES` 渲染），二者非独立一级菜单模块，维持主壳渲染，故未在菜单层面迁移。
