...
- 典型症状②（2026-09-14）：_*大屏设计图整页全裂、后端无任何报错*_ —— PNG→WebP 转换改了 `public/design` 与 `src/utils/designAssets.ts`，却漏重建子应用，旧 bundle 仍请求已删除的 `.png`（3191 处）→ 全部 404。**改 `public/design|images` 或 `designAssets.ts` 后同样必须重建子应用。**
- 排查顺序：先看 dev server 启动日志的 `[subapp-stale]` 警告（列出落后于源码的子应用）；再看子应用 iframe 内请求是否缺 Authorization 头 / 图片是否 404。
- 单个重建省时：`SUBAPP=fm-emergency npm run build:subapps`；避开批量删除守卫：`SUBAPP_NO_EMPTY=1 npm run build:subapps`。
- **门禁**：`npm run gate:subapp-assets` 扫描已构建产物，校验其中的项目资产引用全部可解析（悬空即 FAIL：扩展名漂移 / 文件缺失），CI 已在 build 后 `build:subapps && gate:subapp-assets` 守门（见 `scripts/check-subapp-assets.mjs`）。
...
- **Git 提交规范：`type(scope): 描述`（conventional commits + 端 scope）**。scope 固定枚举，禁止自造：`screen`（大屏壳 + `subapps/`）、`mgmt`（`apps/mgmt`）、`mobile`（`apps/mobile`）、`shared`（`src/` 跨端公共服务、`src/styles/tokens.css`、`vite.config.ts`）、`docs`、`chore`、`contract`（`docs/api` 契约真源与生成类型）、`ci`（CI 流水线与门禁配置）。仓库名（`frontend`）、架构层（`services` / `components`）与工具名（`lint` / `build` / `types`）**不是 scope**：按改动所属端或域取名，跨端则写 `type: 描述` 不带括号。跨端改动**按影响面拆成多个提交**：共享文件（token、公共服务）先行，端内跟随；确属一个原子改动且拆不开时才允许双 scope（如 `feat(mgmt,shared):`），不得常态化。提交信息**只写一行标题**（`type(scope): 描述` 本身就是那句总结），**禁止正文/body**、禁止写一长段描述或用 `- ` 等分点列表展开；说明性内容写进代码注释、`docs/` 或 openspec。禁止提交临时输出文件（如 `tsc-out.txt`、`vitest-out.txt`）。
- 提交前钩子（husky + lint-staged）会执行 eslint / prettier / stylelint，遵循现有 `.prettierrc.json`、`.stylelintrc.json` 配置，不新增例外。
- 上述提交规范已**机控**（`.husky/commit-msg` → `scripts/commit-msg-lint.sh`）：header 格式 + 描述含中文 + scope ∈ 上列枚举 + **body 非空即拒（提交信息必须单行）**。违者提交被拒，**不得用 `--no-verify` 绕过**；确需新 scope 先改本节枚举再写提交。
- 「禁止提交临时输出文件」同样机控（`.husky/pre-commit` → `scripts/pre-commit-guard.sh`）：暂存区命中 `dist/`、`coverage/`、`node_modules/`、`*-out.txt`、`*.log`、`*.tmp` 等即拒绝，`openspec/` 下的 QA 证据附件为例外。
