/* eslint-disable-next-line @typescript-eslint/ban-ts-comment -- 源项目迁移 canvas 绘制代码，保留源类型风格 */
// @ts-nocheck
...
function normalizeCssColor(css) {
  const s = String(css ?? '#377dff').trim();
...
function getFrostedPanelTexture(theme) {
  ...
  rctx.fillStyle = 'rgba(6, 28, 42, 0.72)';
  rctx.fillRect(0, 0, w, h);

  const blobs = [
    [0.18, 0.32, 0.22, 'rgba(28, 72, 58, 0.55)'],
    [0.62, 0.48, 0.18, 'rgba(72, 88, 108, 0.5)'],
    [0.42, 0.72, 0.16, 'rgba(48, 96, 120, 0.45)'],
    [0.78, 0.28, 0.14, 'rgba(90, 70, 50, 0.38)'],
  ];
  ...
  if (bctx) {
    ...
    bctx.fillRect(0, 0, w, h);
    bctx.fillStyle = 'rgba(6, 28, 42, 0.78)';
    bctx.fillRect(0, 0, w, h);
  }
...
export const AI_DIAGNOSIS_ALARM_TAG_THEME = getPlantZoneTagThemeRgb('#dc3737');
...
function drawAlarmDiagnosisPopup(ctx, meta, layoutW, originY) {
  ...
  ctx.save();
  ctx.shadowColor = 'rgba(211, 50, 50, 0.48)';
  ctx.shadowBlur = 14;
  ctx.shadowOffsetY = 2;

  const grad = ctx.createLinearGradient(px, py, px, py + POPUP_H);
  grad.addColorStop(0, 'rgba(48, 10, 14, 0.94)');
  grad.addColorStop(1, 'rgba(24, 6, 10, 0.9)');
  ...
  roundRect(ctx, px, py, POPUP_W, POPUP_H, 5);
  ctx.strokeStyle = 'rgba(255, 110, 110, 0.58)';
  ctx.lineWidth = 1;
  ctx.stroke();
  ...
  ctx.lineTo(tipX, tipY + 8);
  ctx.closePath();
  ctx.fillStyle = 'rgba(24, 6, 10, 0.92)';
  ctx.fill();
  ctx.strokeStyle = 'rgba(255, 110, 110, 0.58)';
  ctx.lineWidth = 1;
  ctx.stroke();
