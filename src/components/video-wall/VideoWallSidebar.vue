...
const filterTree = <T extends { name: string; children: Array<{ name: string }> }>(tree: T[]) => {
...
onMounted(() => {
  try {
    const Cesium =
      (window as unknown as { Cesium?: CesiumNamespace }).Cesium ??
      (window.parent as unknown as { Cesium?: CesiumNamespace }).Cesium;
    if (Cesium && eagleEyeMapRef.value) {
      ...
      if (creditContainer instanceof HTMLElement) creditContainer.style.display = 'none';
...
const getBoundCameras = (targetId: string) => {
  const boundCameras: Array<{ id: string; name: string }> = [];
...
const selectedVideoToBind = ref<{ id: string; name: string } | null>(null);

const openBindMenu = (e: MouseEvent, video: { id: string; name: string }) => {
