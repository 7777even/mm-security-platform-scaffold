...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment -- 源项目迁移代码（约 900 处类型债），本次迁移不改写源类型风格，豁免记录见 openspec/changes/screen-map-base-replace
// @ts-nocheck
...
import { accidentRescueFlyTarget } from '@/composables/sharedCesiumBridge';
import { plantAreaBoundaryRings, plantAreaDefinitions } from '@/services/map-data/plantAreas.ts';
import {
...
import { tvInspectionScanPointsByCircle } from '@/services/map-data/tvMock.ts';
import {
  patchTvInspectionScanState,
  resetTvInspectionScanState,
} from '@/composables/sharedCesiumBridge.ts';
import { setAccidentRescueFlatViewActive } from '@/utils/accidentRescueFlatViewState.ts';
import {
  BOUNDARY_FILL_BLUE,
  BOUNDARY_STROKE_CYAN,
  EVACUATION_BOUNDARY_BLUE,
  EVACUATION_ROUTE_YELLOW,
  FACTORY_AREA_CHEMICAL,
  FACTORY_AREA_FALLBACK,
  FACTORY_AREA_PORT,
  FACTORY_LABEL_BG,
  FACTORY_LABEL_OUTLINE,
  MAP_CANVAS_BASE_BLACK,
  MONITOR_FOCUS_BASE_LINE,
  MONITOR_FOCUS_DASH_CYAN,
  PLANT_EDGE_GLOW,
  PLANT_EDGE_HIGHLIGHT,
  PLANT_EDGE_LINE,
  PLANT_ZONE_PALETTE,
  SCENE_BG_LIGHT,
  SCENE_BG_TECH,
  SCENE_GLOBE_BASE_TECH,
  SIDE_WALL_BOTTOM,
  SIDE_WALL_HIGHLIGHT,
  SIDE_WALL_MID,
  SIDE_WALL_TOP,
  SITE_CENTER_LABEL_COLOR,
  SITE_CENTER_LABEL_OUTLINE,
  SITE_CENTER_POINT_COLOR,
  SITE_CENTER_POINT_OUTLINE,
...
const ACCIDENT_RESCUE_FLAT = {
  ...
  /** HTML 标记相对地形的抬高 */
  markerOffset: 6.5,
  boundaryStroke: BOUNDARY_STROKE_CYAN,
  boundaryFill: BOUNDARY_FILL_BLUE,
...
const MAP_THEME = {
  ...
  colors: {
    plantPalette: PLANT_ZONE_PALETTE,
    edgeLine: PLANT_EDGE_LINE,
    edgeHighlight: PLANT_EDGE_HIGHLIGHT,
    edgeGlow: PLANT_EDGE_GLOW,
    sideWallTop: SIDE_WALL_TOP,
    sideWallHighlight: SIDE_WALL_HIGHLIGHT,
    sideWallMid: SIDE_WALL_MID,
    sideWallBottom: SIDE_WALL_BOTTOM,
  },
};
...
const PLANT_MARKER_SCALE_REF_DISTANCE = MAOMING_PETRO.cameraRange * 0.5;
const PLANT_MARKER_SCALE_MIN_DISTANCE = 120;
const PLANT_MARKER_SCALE_MIN = 0.1;
const PLANT_MARKER_SCALE_MAX = 1;
...
function EvacuationRouteFlowMaterialProperty(options) {
  ...
  this._color =
    options?.color ?? Cesium.Color.fromCssColorString(EVACUATION_ROUTE_YELLOW).withAlpha(0.95);
...
function prepareExportedMapCanvas(image) {
  ...
  ctx.fillStyle = MAP_CANVAS_BASE_BLACK;
...
function _ensureEvacuationBoundaryEntities() {
  ...
  const stroke = Cesium.Color.fromCssColorString(EVACUATION_BOUNDARY_BLUE).withAlpha(0.9);
    ...
    if (boundaryFlatEdgeCartographics?.length) {
      ...
      evacuationOutlineDataSource.entities.add({
        ...
        polyline: {
          ...
          clampToGround: true,
          material: new Cesium.PolylineDashMaterialProperty({
            color: Cesium.Color.fromCssColorString(EVACUATION_BOUNDARY_BLUE).withAlpha(0.95),
            dashLength: 16,
          }),
          disableDepthTestDistance: Number.POSITIVE_INFINITY,
        },
      ...
      evacuationOutlineDataSource.entities.add({
        ...
        polyline: {
          ...
          material: new Cesium.PolylineGlowMaterialProperty({
            glowPower: 0.12,
            taperPower: 0.55,
            color: Cesium.Color.fromCssColorString(EVACUATION_BOUNDARY_BLUE).withAlpha(0.35),
          }),
          disableDepthTestDistance: Number.POSITIVE_INFINITY,
        },
      });
...
function applyTechSceneStyle(viewer) {
  const { scene } = viewer;
  scene.backgroundColor = Cesium.Color.fromCssColorString(SCENE_BG_TECH);
  scene.globe.baseColor = Cesium.Color.fromCssColorString(SCENE_GLOBE_BASE_TECH);
...
function _applyLightSceneStyle(viewer) {
  const { scene } = viewer;
  scene.backgroundColor = Cesium.Color.fromCssColorString(SCENE_BG_LIGHT);
  scene.globe.baseColor = Cesium.Color.fromCssColorString(SCENE_BG_LIGHT);
...
function showEvacuationRoute(route) {
  ...
  const _baseColor = Cesium.Color.fromCssColorString(EVACUATION_ROUTE_YELLOW).withAlpha(0.28);
  const glowColor = Cesium.Color.fromCssColorString(EVACUATION_ROUTE_YELLOW).withAlpha(0.55);
    ...
    evacuationRouteDataSource.entities.add({
      ...
      polyline: {
        ...
        material: createEvacuationRouteFlowMaterial({
          color: Cesium.Color.fromCssColorString(EVACUATION_ROUTE_YELLOW).withAlpha(0.95),
          speed: 0.9,
          repeat: 18.0,
        }),
      },
...
function setMonitoringFocusArea(enable, target = null) {
  ...
  monitoringFocusDataSource.entities.add({
    ...
    polyline: {
      positions,
      width: 7,
      material: Cesium.Color.fromCssColorString(MONITOR_FOCUS_BASE_LINE).withAlpha(0.82),
      disableDepthTestDistance: Number.POSITIVE_INFINITY,
    },
  });
  monitoringFocusDataSource.entities.add({
    ...
    polyline: {
      ...
      width: 3,
      material: new Cesium.PolylineDashMaterialProperty({
        color: Cesium.Color.fromCssColorString(MONITOR_FOCUS_DASH_CYAN).withAlpha(0.98),
        dashLength: 18,
      }),
      disableDepthTestDistance: Number.POSITIVE_INFINITY,
    },
...
function factoryAreaTheme(code, selected) {
  const colors = { chemical: FACTORY_AREA_CHEMICAL, port: FACTORY_AREA_PORT };
  const base = Cesium.Color.fromCssColorString(colors[code] ?? FACTORY_AREA_FALLBACK);
  return {
  ...
  labelPoints.forEach((item) => {
    viewer.entities.add({
      ...
      label: {
        ...
        outlineColor: Cesium.Color.fromCssColorString(FACTORY_LABEL_OUTLINE),
        outlineWidth: 4,
        style: Cesium.LabelStyle.FILL_AND_OUTLINE,
        showBackground: true,
        backgroundColor: Cesium.Color.fromCssColorString(FACTORY_LABEL_BG).withAlpha(0.86),
...
onMounted(async () => {
  ...
  try {
    ...
    viewer.entities.add({
      ...
      point: {
        pixelSize: 8,
        color: Cesium.Color.fromCssColorString(SITE_CENTER_POINT_COLOR),
        outlineColor: Cesium.Color.fromCssColorString(SITE_CENTER_POINT_OUTLINE),
        outlineWidth: 1,
      },
      label: {
        text: '茂名石化',
        font: '12px Microsoft YaHei, sans-serif',
        fillColor: Cesium.Color.fromCssColorString(SITE_CENTER_LABEL_COLOR),
        outlineColor: Cesium.Color.fromCssColorString(SITE_CENTER_LABEL_OUTLINE),
        outlineWidth: 2,
        style: Cesium.LabelStyle.FILL_AND_OUTLINE,
        verticalOrigin: Cesium.VerticalOrigin.BOTTOM,
