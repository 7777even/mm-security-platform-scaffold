...
import { TYPHOON_POPUP_LABEL, TYPHOON_POPUP_TEXT } from '@/constants/mapScene';
import {
  fetchTyphoonDetail,
...
} from '@/services/weather/istrongcloudTyphoonApi';
...
export function formatTyphoonPopupHtml(point: TyphoonTrackPointInfo): string {
  ...
  return `
    <div style="min-width:180px;font-size:12px;line-height:1.5;color:${TYPHOON_POPUP_TEXT};">
      ${rows
        .map(
          ([label, value]) =>
            `<div style="display:flex;justify-content:space-between;gap:12px;padding:2px 0;"><span style="color:${TYPHOON_POPUP_LABEL};">${label}</span><strong style="font-weight:500;">${value}</strong></div>`,
        )
        .join('')}
    </div>
  `;
