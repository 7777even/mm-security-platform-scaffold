...
// 排空微任务队列（Promise 续期链依赖微任务，与 fake timers 无关）
...
describe('realtime-channel 能力', () => {
  ...
  describe('WS 握手鉴权（?token= 注入）', () => {
    it('getToken 返回令牌时注入 ?token= 到连接 URL', () => {
      const client = createClient({ getToken: () => 'abc123' });
      client.connect();
      const s = FakeSocket.instances[0]!;
      expect(s.url).toBe('ws://test.local?token=abc123');
    });

    it('getToken 返回 null（未登录）时不注入令牌', () => {
      const client = createClient({ getToken: () => null });
      client.connect();
      const s = FakeSocket.instances[0]!;
      expect(s.url).toBe('ws://test.local');
    });

    it('URL 已带查询参数时以 & 追加 token', () => {
      const client = new RealtimeClient({
        url: 'ws://test.local?foo=1',
        createSocket: (url: string) => new FakeSocket(url),
        getToken: () => 'tk',
      });
      client.connect();
      expect(FakeSocket.instances[0]!.url).toBe('ws://test.local?foo=1&token=tk');
    });
  });

  describe('鉴权失败刷新重连（防无限锤击）', () => {
    it('从未连上且刷新成功 → 刷新一次后立即重建连接', async () => {
      const refreshToken = vi.fn().mockResolvedValue(true);
      const client = createClient({ getToken: () => 'tk', refreshToken });
      client.connect();
      const s1 = FakeSocket.instances[0]!;
      expect(s1.url).toContain('token=tk');
      // 模拟握手被拒（从未成功打开）
      s1.fail();
      ...
      expect(s2.url).toContain('token=tk');
    });

    it('从未连上且刷新失败 → 不立即重建，退回退避重连且刷新仅一次', async () => {
    ...
    it('已成功打开后掉线 → 不再触发令牌刷新（防瞬时抖动刷爆续期）', () => {
      const refreshToken = vi.fn().mockResolvedValue(true);
      const client = createClient({ getToken: () => 'tk', refreshToken });
      client.connect();
      const s1 = FakeSocket.instances[0]!;
