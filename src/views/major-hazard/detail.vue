...
function goBack() {
  void router.push({ name: 'ops-monitor-hazards' });
}
...
.hazard-detail__rows {
  ...
  gap: 0;
  margin-bottom: 14px;
  border: 1px solid color-mix(in srgb, var(--map-border) 18%, transparent);
  border-radius: 2px;
...
.hazard-detail__row {
  ...
  gap: 8px;
  padding: 8px 12px;
  border-bottom: 1px solid color-mix(in srgb, var(--map-border) 14%, transparent);
  font-size: 12px;
}
...
.hazard-detail__section-title {
  margin-bottom: 8px;
  font-size: 13px;
  color: color-mix(in srgb, var(--map-sky) 92%, transparent);
}
