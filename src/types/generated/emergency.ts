...
export interface components {
  schemas: {
    ...
    EmergencyResource: {
      ...
      icon?: string;
      /**
       * @description 真实明细预览（仅应急专家/应急物资/应急车辆/救援队伍 4 类有值，其余类别为 null），取各台账前 20 条
       * @example [
       *       {
       *         "name": "张伟",
       *         "meta": "救援专家 · 一中队"
       ...
       *     ]
       */
      items?: components['schemas']['StrengthItem'][];
    };
    StrengthItem: {
      /**
       * @description 明细名称（专家姓名 / 装备名 / 车牌 / 队伍名）
       * @example 张伟
       */
      name?: string;
      /**
       * @description 辅助说明（岗位 / 规格 / 车型 / 区域），无则为空
       * @example 救援专家 · 一中队
       */
      meta?: string;
    };
